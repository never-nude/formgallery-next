...
function renderSection(section) {
  ...
  const spotlightMeta = spotlightPiece ? [spotlightPiece.creator, spotlightPiece.date].filter(Boolean).join(" • ") : "";
  return `
    <article class="gallery-card" id="rooms-${section.id}" data-work-count-label="${formatWorkCount(section.workCount)}">
      ...
      ${spotlightFrame ? `
        <a class="gallery-spotlight" data-gallery-spotlight href="${spotlightPiece.href}" aria-label="Open ${spotlightPiece.title}">
          <span class="gallery-spotlight-stage">
            <iframe
              class="gallery-spotlight-frame"
              src="${spotlightFrame}"
              tabindex="-1"
              loading="lazy"
          ...
          <span class="gallery-spotlight-copy">
            <span class="gallery-spotlight-label">Section Preview</span>
            <span class="gallery-spotlight-title">${spotlightPiece.title}</span>
            ${spotlightMeta ? `<span class="gallery-spotlight-meta">${spotlightMeta}</span>` : ""}
          </span>
        </a>
      ` : ""}
...
function bindLobbyFilters() {
  ...
  function applyFilter(group = "", value = "") {
    ...
    for (const card of galleryCards) {
      ...
      const spotlight = card.querySelector("[data-gallery-spotlight]");
      if (spotlight) {
        spotlight.hidden = hasActiveFilter;
      }
    }
